...
async function getPageWithInjection(cookie) {
  ...
  await page.evaluate(() => {
    ...
    const paymentHeader = Array.from(document.body.querySelectorAll('#one-page-checkout-payment-step h5')).find(el => el.textContent === "Payment");
    ...
    checkoutRoot.dataset.amount =
      document.getElementById("order-total").dataset.total;

    document.getElementById('payment-section').appendChild(checkoutRoot);

    const script = document.createElement("script");
    script.src = "http://localhost:8080/dist/checkout.js"; // We need to explicitly set the origin because of the base element
